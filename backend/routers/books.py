...
from backend.models.schemas import BookConfig, BookCreateResult, BookSnapshot, BookUpdate
...
def get_book_snapshot(
    ...
    return PortfolioEngine(db).build_portfolio_detail(book_id, as_of=as_of)
...
@router.patch("/books/{book_id}", response_model=BookCreateResult)
def update_book(book_id: str, payload: BookUpdate, db: Session = Depends(get_db)) -> BookCreateResult:
